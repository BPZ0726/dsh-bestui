/* GENERATED FILE — do not edit. Run `node scripts/build.mjs` to rebuild.
...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    const STORAGE_KEY = "dsh-wallpaper-theme/v1";
    ...
    const SURFACE_IDS = TOKEN_DEFS.filter((def) => def.kind === "surface").map((def) => def.id);
    const BORDER_IDS = TOKEN_DEFS.filter((def) => def.kind === "border").map((def) => def.id);
    const ALL_TOKEN_IDS = TOKEN_DEFS.map((def) => def.id);
     ...
     */
    const PINNED_OPAQUE_SURFACES = new Set([
    ...
    const OPACITY_IDS = SURFACE_IDS.filter((id) => !PINNED_OPAQUE_SURFACES.has(id));
    ...
    function pickerToCss(color, alpha) {
    ...
    function sampleImageDataUrl(dataUrl) {
    ...
    function paletteFromSample(sample) {
     ...
     */
    function tintHex(hex, strength, cap, lShift) {
    ...
    function rotateHue(hex, shiftDeg) {
      const rgb = hexToRgb(hex);
      if (!rgb) return hex;
      const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);
      if (hsl.s === 0) return hex;
     ...
     */
    function appearanceIsStock(state) {
     ...
     */
    function compressDataUrl(dataUrl, mime) {
      return new Promise((resolve) => {
     ...
     */
    function paintFontSubtree(root, scale, parentRef) {
     ...
     */
    function createRuntime({ theme, insertCss, console: logger }) {
...
function apply(ctx) {
  var api = createRuntime({ theme: ctx.theme, insertCss: insertCss, console: console });
  ...
  return disposeWallpaperTheme;
    ...
    exports.meta = {
  "name": "BestUI",
  "description": "Wallpaper background theme for the dsh web GUI: image background, per-surface opacity, per-element light/dark colors, auto palettes from the image or the browser color scheme."
};
